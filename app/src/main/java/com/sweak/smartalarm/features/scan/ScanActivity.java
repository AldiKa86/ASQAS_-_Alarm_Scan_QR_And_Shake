...
import com.sweak.smartalarm.features.shake.ShakeActivity;
import com.sweak.smartalarm.features.shake.ShakeWqrActivity;
...
public class ScanActivity extends AppCompatActivity {
        ...
        mCodeScanner.setDecodeCallback(result -> runOnUiThread(() -> {
            if (mScanMode == MODE_DISMISS_ALARM) {
                if (result.getText().equals(mPreferences.getDismissAlarmCode())) {
                    ...
                    wrongScanCount++;
                    if (wrongScanCount >= 3) {
                        Intent intent = new Intent(this, ShakeWqrActivity.class);
                        startActivity(intent);
                        finish();
                    }
                }
    ...
    private void showWrongCodeToast(String code) {
        Toast.makeText(this,
                getString(R.string.wrong_code) + " \"" + code + "\" " + getString(R.string.tap_to_try_again),
                Toast.LENGTH_LONG)
                .show();
    }

//    private void showWrongCodeRepeatToast (String text) {
//        Toast.makeText(this,
