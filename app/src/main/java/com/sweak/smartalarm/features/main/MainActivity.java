...
import com.sweak.smartalarm.features.scan.CustomActivity;
...
public class MainActivity extends AppCompatActivity
    ...
    private void enableSetAlarm() {
        mBinding.alarmText.setText(R.string.set_alarm);
    }

    private void enableAlarmSet() {
        mBinding.alarmText.setText(R.string.alarm_set);
    }

    private void enableInfoButton() {
        mBinding.infoButton.setClickable(true);

        mBinding.infoButton.startAnimation(mAnimationIn);
        new Handler().postDelayed(() -> mBinding.infoButton.setVisibility(View.VISIBLE), 1000);
    }

    private void disableInfoButton() {
        mBinding.infoButton.setClickable(false);

        mBinding.infoButton.startAnimation(mAnimationOut);
        new Handler().postDelayed(() -> mBinding.infoButton.setVisibility(View.INVISIBLE), 1000);
    }
    ...
    public void snoozeAlarm(View view) {
        Intent snoozeIntent = new Intent(ACTION_SNOOZE).setClassName(/* TODO: provide the application ID. For example: */ getPackageName(), "com.sweak.smartalarm.receiver.SnoozeReceiver");
